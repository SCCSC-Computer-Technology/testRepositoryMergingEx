    ...
    public partial class InspectionReport : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //string dateRange = selectedDate.ToString("yyyy-MM-dd");

                // Query the database or perform any necessary data retrieval logic
                //DataTable data = GetDataFromDatabase(startDate,endDate, selectedVehicle, selectedUser);

                DataTable data = null;

                if (selectVehicle)
                {
                    data = GetDataFromDatabase(startDate, endDate, null, null);
                }
                else
                {
                    data = GetDataFromDatabase(startDate, endDate, selectedVehicle, null);
                }
        ...
        private DataTable GetDataFromDatabase(DateTime startDate,DateTime endDate, string selectedVehicle, string selectedUser)
        {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("inspection_report", connection))
                    {
                    ...
                    {
                        command.Parameters.AddWithValue("@selected_vehicle", selectedVehicle);
                    }
